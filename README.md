...
└─ RagEngine                 문서 추가(addDocument) / 검색(search) 파사드
   ...
   ├─ Embedder               텍스트 → 벡터 인터페이스
   │   └─ HashingEmbedder    Phase 0 기본 구현(문자 n-gram 해싱, 오프라인·무모델)
   │       (다음 슬라이스: ONNX Runtime + multilingual-e5-small 교체 예정)
   └─ NativeVectorIndex      JNI 브리지 (FloatArray ↔ C++)
       └─ librag-core.so     C++17 코사인 유사도 브루트포스 top-k
                             (Phase 1에서 HNSW로 교체 예정)
```
...
| 단계 | 내용 | 상태 |
|---|---|---|
| Phase 0 | JNI 뼈대 + C++ 코사인 top-k + 청킹/검색 파이프라인 | ✅ 진행 중 |
| Phase 0.5 | ONNX Runtime + multilingual-e5-small 임베딩 교체 | ⏳ |
| Phase 1 | C++ HNSW 벡터 인덱스 | ⏳ |
